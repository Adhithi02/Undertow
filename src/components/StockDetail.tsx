...
type MarketContext = { analystConsensus: { period: string; strongBuy: number; buy: number; hold: number; sell: number; strongSell: number; total: number } | null; source: string | null; fetchedAt: string | null; disclosure: string };
...
function AnalystContext({ context }: { context: MarketContext | null }) {
  if (!context) return null;
  const consensus = context.analystConsensus;
  return <section className="market-context" aria-labelledby="market-context-heading"><div className="market-context-heading"><div><p className="mono">Real data context</p><h2 id="market-context-heading">Analyst recommendation trend</h2></div><span>{context.source ?? "Provider unavailable"}</span></div>{consensus ? <><div className="recommendation-bars" aria-label={`Finnhub analyst recommendation counts for ${consensus.period}`}><span className="strong-buy" style={{ flex: consensus.strongBuy }}>Strong buy <b>{consensus.strongBuy}</b></span><span className="buy" style={{ flex: consensus.buy }}>Buy <b>{consensus.buy}</b></span><span className="hold" style={{ flex: consensus.hold }}>Hold <b>{consensus.hold}</b></span><span className="sell" style={{ flex: consensus.sell + consensus.strongSell }}>Sell <b>{consensus.sell + consensus.strongSell}</b></span></div><div className="market-context-meta"><span>{consensus.total} published recommendations · period {consensus.period}</span><span>Fetched {context.fetchedAt ? new Date(context.fetchedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "now"}</span></div></> : <p className="market-context-unavailable">Finnhub recommendation trends are not available for this symbol or API plan right now. The modelled analyst signal remains separate.</p>}<p className="market-context-disclosure">{context.disclosure}</p></section>;
}
